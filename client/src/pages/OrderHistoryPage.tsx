      ...
      <div className="px-6 sm:px-[clamp(1rem,11.40vw,200px)]">
        ...
        {!error && orders.length === 0 && (
          <div className="bg-white rounded-2xl p-12 text-center">
           ...
           px-8 py-4 hover:bg-[#FBAF85] transition-colors duration-200 cursor-pointer inline-block"
            >
              Shop Now
            </Link>
          </div>
                ...
                <div className="flex flex-col gap-4">
                  {order.cartItems.map((item, i) => (
                    <div key={i} className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-[#F1F1F1] rounded-xl flex items-center justify-center shrink-0">
                        <img
                          src={item.image}
                          alt={item.name}
